...
<div id="root-packages">
	...
	<div class="section packages-content">
		<h3>
		<div>选择密钥种类</div>
		<small style="color:#1aa79a"><?php if(!$GLOBALS['userlogined']){echo '您登录后购买的密钥将显示在此处';}elseif(is_file(IN_ROOT.'./data/tmp/buy_key_'.$GLOBALS['erduo_in_userid'].'.txt')){echo '您最近购买的密钥为【<b style="color:#ec4242">'.file_get_contents(IN_ROOT.'./data/tmp/buy_key_'.$GLOBALS['erduo_in_userid'].'.txt').'</b>】，请及时保存或使用';}else{echo '您最近购买的密钥将显示在此处';} ?></small>
		</h3>
		...
		<small>如需线下付款购买，请联系&nbsp;<a href="mailto:<?php echo IN_MAIL; ?>"><?php echo IN_MAIL; ?></a></small>
	</div>
...
<div class="dialog buy-confirm" style="display:none">
	...
	<div class="actions text-center">
		<button class="btn btn-default" style="margin-bottom:10px" onclick="$('.dialog-mask').hide(),$('.buy-confirm').hide()">放弃购买</button><button class="btn btn-yellow" style="margin-bottom:10px" onclick="location.reload()">购买成功，马上查看</button>
	</div>
</div>
