...
function CheckForm(){
        if(document.form2.in_name.value==""){
            ...
            asyncbox.tips("配置地址不能为空，请填写！", "wait", 1000);
...
<option value="1"<?php if($in_type==1){echo " selected";} ?>>企业版</option>
</select></td>
<td class="td29">版本兼容：<input type="text" class="txt" value="<?php echo $in_mnvs; ?>" name="in_mnvs" id="in_mnvs"></td>
</tr>
<tr><td class="td29">应用平台：<input type="text" class="txt" value="<?php echo $in_form; ?>" name="in_form" id="in_form"></td><td class="td29">应用大小：<input type="text" class="txt" value="<?php echo $in_size; ?>" name="in_size" id="in_size"></td></tr>
...
<tr><td class="longtxt diffcolor3">配置地址：<input type="text" class="txt" value="<?php echo $in_plist; ?>" name="in_plist" id="in_plist"></td><td><div class="rssbutton"><input type="button" value="上传应用" onclick="pop.up('no', '上传应用', 'source/pack/upload/admin-open.php', '406px', '180px', '175px');" /></div></td></tr>
...
<td><?php if($row['in_sign']){echo '<em class="lightnum">已开通('.$row['in_resign'].')</em>';}else{echo '未开通';} ?></td>
...
<?php
	...
	function AllDel(){
		...
		if($in_id==0){
			...
			if($db->query($sql)){
				$db->query("delete from ".tname('signlog')." where in_status=1 and in_aid in ($in_id)");
