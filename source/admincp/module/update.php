...
function change_data(){
	...
	if(!preg_match("/define\('IN_WEBVIEWPOINTS'/", $config)){
	        $replace = "define('IN_WEBVIEWPOINTS', '3000');\r\n";
	        $replace = $replace."define('IN_ADPOINTS', '0');\r\n";
	        $replace = $replace."define('IN_ADLINK', '');\r\n";
	        $replace = $replace."define('IN_ADIMG', '');\r\n";
	        $replace = $replace."define('IN_REMOTE'";
	        $config = preg_replace("/define\('IN_REMOTE'/", $replace, $config);
	        $GLOBALS['db']->query("ALTER TABLE `".IN_DBTABLE."app` ADD `in_removead` int(11) NOT NULL");
	}
