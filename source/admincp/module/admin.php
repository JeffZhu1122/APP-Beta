...
<tr><td class="vtop rowform"><input type="password" class="txt" name="in_adminpassword" id="in_adminpassword"></td><td class="vtop tips2"><?php if($_GET['action']=="edit"){echo "不修改请留空";} ?></td></tr>
...
<tr>
<td class="vtop"><ul>
<?php if(ergodic_array($in_permission,1)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value1" value="1"<?php if(ergodic_array($in_permission,1)){echo " checked";} ?>><label for="value1">首页</label></li>
<?php if(ergodic_array($in_permission,2)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value2" value="2"<?php if(ergodic_array($in_permission,2)){echo " checked";} ?>><label for="value2">全局</label></li>
<?php if(ergodic_array($in_permission,3)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value3" value="3"<?php if(ergodic_array($in_permission,3)){echo " checked";} ?>><label for="value3">应用</label></li>
<?php if(ergodic_array($in_permission,4)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value4" value="4"<?php if(ergodic_array($in_permission,4)){echo " checked";} ?>><label for="value4">用户</label></li>
<?php if(ergodic_array($in_permission,5)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value5" value="5"<?php if(ergodic_array($in_permission,5)){echo " checked";} ?>><label for="value5">工具</label></li>
<?php if(ergodic_array($in_permission,6)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value6" value="6"<?php if(ergodic_array($in_permission,6)){echo " checked";} ?>><label for="value6">系统</label></li>
<?php if(ergodic_array($in_permission,7)){echo "<li class=\"checked\">";}else{echo "<li>";} ?><input class="checkbox" type="checkbox" name="in_permission[]" id="value7" value="7"<?php if(ergodic_array($in_permission,7)){echo " checked";} ?>><label for="value7">云平台</label></li>
</ul></td>
...
<?php
	...
	function SaveEdit(){
		...
		if($in_adminpassword!==$in_adminpassword1){ShowMessage("编辑失败，两次密码填写不一致！","history.back(1);","infotitle3",3000,2);}
		if($db->getone("select in_adminid from ".tname('admin')." where in_adminid<>".$in_adminid." and in_adminname='".$in_adminname."'")){ShowMessage("编辑出错，该帐号已经存在！","history.back(1);","infotitle3",3000,2);}
	...
	function Del(){
		global $db;
		if(!submitcheck('hash', -1)){ShowMessage("链接来路不明，无法提交！",$_SERVER['PHP_SELF'],"infotitle3",3000,1);}
		$in_adminid=intval(SafeRequest("in_adminid","get"));
		if($in_adminid==1){ShowMessage("抱歉，默认帐号不允许删除！","?iframe=admin","infotitle3",3000,1);}
	...
	function SaveAdd(){
		...
		$in_islock=SafeRequest("in_islock","post");
		$in_permission=RequestBox("in_permission");
		if(empty($in_adminpassword) || $in_adminpassword!==$in_adminpassword1){ShowMessage("新增失败，密码为空或两次密码填写不一致！","history.back(1);","infotitle3",3000,2);}
