...
([\s\S]+?)<string>([\s\S]+?)<\/string>/', $em, $m) ? mb_convert_encoding($m[2], set_chars(), 'HTML-ENTITIES') : '*';
	...
	fwrite(fopen($path.'.plist', 'w'), convert_charset($str));
		...
		$GLOBALS['db']->query("Insert ".tname('app')." (in_name,in_uid,in_uname,in_type,in_size,in_form,in_mnvs,in_bid,in_bsvs,in_bvs,in_nick,in_team,in_icon,in_plist,in_hits,in_kid,in_sign,in_resign,in_removead,in_addtime) values ('$xml_name',".$GLOBALS['erduo_in_userid'].",'".$GLOBALS['erduo_in_username']."',$xml_type,'$xml_size','$xml_form','$xml_mnvs','$xml_bid','$xml_bsvs','$xml_bvs','$xml_nick','$xml_team','$xml_icon','$xml_plist',0,0,0,0,0,'".date('Y-m-d H:i:s')."')");
        }
	echo '1';
}
?>